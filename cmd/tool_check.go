...
func (s *checkCommand) Setup() (err error) {
	if tool, ok := cmdWithFullCommand("tool"); ok {
		s.cmd = tool.Cmd().Command("check", "Checks the health of a running Choria instance based on its status file")
...
func (s *checkCommand) Configure() error {
	return nil
}

func (s *checkCommand) checkConnection(status *server.InstanceStatus) (err error) {
	if !s.checkConnected {
		return nil
	}
...
func (s *checkCommand) checkLastMessage(status *server.InstanceStatus) (err error) {
	if s.lastMessage == 0 {
		return nil
	}
...
func (s *checkCommand) checkFileAge() (err error) {
	if s.maxAge == 0 {
		return nil
	}
...
func (s *checkCommand) Run(wg *sync.WaitGroup) (err error) {
	defer wg.Done()
...
func (s *checkCommand) exit(err error) {
	if err != nil {
		fmt.Printf("%s %s\n", s.statusFile, err)
		os.Exit(1)
	}
...
func init() {
	cli.commands = append(cli.commands, &checkCommand{})
}
