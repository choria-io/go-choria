...
func Must(fw ChoriaFramework, opts ...InitializationOption) (client *AaaSignerClient) {
	c, err := New(fw, opts...)
	if err != nil {
		panic(err)
	}
...
func New(fw ChoriaFramework, opts ...InitializationOption) (client *AaaSignerClient, err error) {
	c := &AaaSignerClient{
		fw:            fw,
		ddl:           &agent.DDL{},
		clientRPCOpts: []rpcclient.RequestOption{},
