...
func Must(fw ChoriaFramework, opts ...InitializationOption) (client *ScoutClient) {
	c, err := New(fw, opts...)
	if err != nil {
		panic(err)
	}
...
func New(fw ChoriaFramework, opts ...InitializationOption) (client *ScoutClient, err error) {
	c := &ScoutClient{
		fw:            fw,
		ddl:           &agent.DDL{},
		clientRPCOpts: []rpcclient.RequestOption{},
