...
// InventoryNS is a NodeSource that uses Choria inventory to discover nodes
type InventoryNS struct {
	nodeCache []string
	f         *protocol.Filter
...
func (b *InventoryNS) Discover(ctx context.Context, fw inter.Framework, filters []FilterFunc) ([]string, error) {
	b.Lock()
	defer b.Unlock()
	...
	source := fw.Configuration().Choria.InventoryDiscoverySource
	if source == "" {
		return nil, fmt.Errorf("inventory discovery source is not set, cannot discover nodes")
	}
	...
	nodes, err := inventory.New(fw).Discover(ctx, inventory.Filter(b.f), inventory.File(source))
	if err != nil {
		return []string{}, err
	}
