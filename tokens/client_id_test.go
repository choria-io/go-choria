...
var _ = Describe("ClientIDClaims", func() {
	...
	BeforeEach(func() {
		...
		claims, err = NewClientIDClaims("up=ginkgo.expired", []string{"rpcutil"}, "choria", map[string]string{"group": "admins"}, "// opa policy", "Ginkgo", -1*time.Hour)
		Expect(err).ToNot(HaveOccurred())
	...
	Describe("NewClientIDClaims", func() {
		It("Should set an issuer when none is given", func() {
			claims, err := NewClientIDClaims("up=ginkgo", []string{"rpcutil"}, "choria", map[string]string{"group": "admins"}, "// opa policy", "", time.Hour)
			Expect(err).ToNot(HaveOccurred())
			Expect(claims.Issuer).To(Equal("Choria"))
		})

		It("Should ensure a callerid is set", func() {
			claims, err := NewClientIDClaims("", []string{"rpcutil"}, "choria", map[string]string{"group": "admins"}, "// opa policy", "", time.Hour)
