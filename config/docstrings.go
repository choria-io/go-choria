...
var docStrings = map[string]string{
	...
	"plugin.choria.network.tls_timeout":                            "Affects only the Network Broker (NATS server). Time to allow for TLS connections to establish, increase on slow or very large networks. It must not exceed 60 seconds.",
	"plugin.choria.network.auth_timeout":                           "Affects only the Network Broker (NATS server). Time to allow for Choria Server (NATS clients) to authenticate on the Network Broker and it is scheduled after the TLS handshake, increase on slow or very large networks. It must not exceed 60 seconds.",
	"plugin.choria.network.connect_timeout":                        "Affects only Choria Servers (NATS clients). Maximum time the NATS client will wait to establish a full connection to the Network Broker (NATS server), including the TCP connection, TLS handshake, and authorization. Increase this value on slow or very large networks. It should be generally larger than plugin.choria.network.tls_timeout + plugin.choria.network.auth_timeout. It must not exceed 120 seconds.",
	...
	"plugin.choria.registration.size_trigger":                      "Enables a trigger that will publish a registration message if the size of the message has changed by greater than the trigger amount in bytes",
	"plugin.choria.registration.size_interval":                     "When the RegistrationSizeTrigger is defined, this property will be used to define how often we check for a change in message size. Default value is 30 seconds",
