...
type ChoriaPluginConfig struct {
	...
	FederationWorkers         int      `confkey:"plugin.choria.federation.workers" default:"10" url:"https://choria.io/docs/federation/"`                                               // The number of concurrent workers each Federation Broker processing pool runs
	...
	NetworkTLSTimeout                  time.Duration `confkey:"plugin.choria.network.tls_timeout" type:"duration" default:"2s"`                                    // Affects only the Network Broker (NATS server). Time to allow for TLS connections to establish, increase on slow or very large networks. It must not exceed 60 seconds.
	NetworkAuthTimeout                 time.Duration `confkey:"plugin.choria.network.auth_timeout" type:"duration" default:"2s"`                                   // Affects only the Network Broker (NATS server). Time to allow for Choria Server (NATS clients) to authenticate on the Network Broker and it is scheduled after the TLS handshake, increase on slow or very large networks. It must not exceed 60 seconds.
	NetworkConnectTimeout              time.Duration `confkey:"plugin.choria.network.connect_timeout" type:"duration" default:"2s"`                                // Affects only Choria Servers (NATS clients). Maximum time the NATS client will wait to establish a full connection to the Network Broker (NATS server), including the TCP connection, TLS handshake, and authorization. Increase this value on slow or very large networks. It should be generally larger than plugin.choria.network.tls_timeout + plugin.choria.network.auth_timeout. It must not exceed 120 seconds.
